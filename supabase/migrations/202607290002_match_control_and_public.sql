...
create index if not exists races_collection_enabled_idx
  on public.races(collection_enabled, sailfish_status)
...
as $$
  with live_and_waiting as (
    ...
    where m.is_public and r.sailfish_status in ('10', '50')
  ),
  waiting_for_next as (
  ...
  history_ready as (
    ...
    from public.races r
    join public.matches m on m.match_cd = r.match_cd
    where m.is_public and r.sailfish_status = '99' and r.history_imported_at is not null
  ),
  catalog as (
